...
void Mission::startSearch(const std::string &agentsFile)
{
    ...
    for (int i = minAgents; i <= config.maxAgents; ++i) {
        ...
        const double time_before = GetMonotonicTime();
        if (config.searchType == CN_ST_PR) {
            pushAndRotate->clear();
        ...
        const double time_after = GetMonotonicTime();
        std::cout << "Path found: " << (sr.pathfound ? "true" : "false") << std::endl;
        if (!sr.pathfound) {
            break;
        }
        ...
        std::cout << "Runtime: " << (time_after - time_before) << std::endl;
        std::cout << "Cost: " << timeflow << std::endl;
