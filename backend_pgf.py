...
def get_fontspec():
    ...
    if texcommand != "pdflatex":
        # latex_fontspec.append("\\usepackage{unicode-math}")
        latex_fontspec.append("\\usepackage{amssymb,amsmath,amsthm,amsfonts,textcomp}")
    ...
    # HACK
    # latex_fontspec.append(r"\%s{%s}[Path=%s]" % (
    #             "setmathrm", "cmr10.ttf", path.parent.as_posix() + "/"))
    # latex_fontspec.append(r"\%s{%s}" % (
    #             "setallmainfonts", "DejaVuSerif.ttf"))#, path.parent.as_posix() + "/"))
    # path = pathlib.Path(fm.findfont("serif"))
    latex_fontspec.append(r"\%s{%s}" % (
                "setallmainfonts", "cmr10.ttf"))#, path.parent.as_posix() + "/"))
...
def _text_cleanup(text):
    text = text.replace(". ", ".\\ ")
    return text.replace("-", "\\unaryminus{}")
